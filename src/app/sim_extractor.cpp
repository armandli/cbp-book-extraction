...
void book_extraction(const s::string& product, int level, int interval, int total, const s::string& prefix, int epoch){
  ...
  outss << prefix << "_" << product << "_book_" << s::to_string(epoch) << ".json";
...
void product_extraction(const s::string& product, const s::string& detail, int interval, int total, const s::string& prefix, int epoch){
  s::stringstream outss;
  outss << prefix << "_" << product << "_" << detail << "_" << s::to_string(epoch) << ".json";
  s::string outfile = outss.str();
  ...
  s::stringstream urlss;
  urlss << COINBASE_URL << product << "/" << detail;
  s::string url = urlss.str();

  CURL* hnd = curl_easy_init();

  if (not hnd){
    BOOST_LOG_SEV(logger(), sev::error) << __FUNCTION__ << " CURL handle cannot be allocated. product: " << product << " detail: " << detail << " prefix: " << prefix;
    return;
  }
  ...
  for (int i = 0; i < total; ++i){
    ...
    if (ret != CURLE_OK){
      BOOST_LOG_SEV(logger(), sev::error) << __FUNCTION__ << " " << detail << " CURL return: " << ret;
      usleep(interval);
      continue;
    }
    ...
    if (chunk.size <= 1){
      BOOST_LOG_SEV(logger(), sev::info) << __FUNCTION__ << " " << detail << "0 chunk size";
      continue;
    }
    ofile << chunk.response;
    ofile.flush();
